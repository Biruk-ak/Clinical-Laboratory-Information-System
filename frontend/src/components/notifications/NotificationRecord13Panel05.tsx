...
export interface NotificationRecord13Panel05Props {
  facilityId: string;
  title?: string;
...
/**
 * NotificationRecord13Panel05 — operational panel for notifications domain slice 13/05.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function NotificationRecord13Panel05({ facilityId, title = 'NotificationRecord13 Workspace', onSelect }: NotificationRecord13Panel05Props) {
  ...
  return (
    <section className="lis-panel lis-panel--notifications" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} notifications records (slice 13-05)
      </footer>
    </section>
  );
}

export default NotificationRecord13Panel05;
