...
export interface NotificationRecord13Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * NotificationRecord13Panel01 — operational panel for notifications domain slice 13/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function NotificationRecord13Panel01({ facilityId, title = 'NotificationRecord13 Workspace', onSelect }: NotificationRecord13Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--notifications" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} notifications records (slice 13-01)
      </footer>
    </section>
  );
}

export default NotificationRecord13Panel01;
