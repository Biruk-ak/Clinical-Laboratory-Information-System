...
export interface NotificationRecord26Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * NotificationRecord26Panel01 — operational panel for notifications domain slice 26/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function NotificationRecord26Panel01({ facilityId, title = 'NotificationRecord26 Workspace', onSelect }: NotificationRecord26Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--notifications" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} notifications records (slice 26-01)
      </footer>
    </section>
  );
}

export default NotificationRecord26Panel01;
