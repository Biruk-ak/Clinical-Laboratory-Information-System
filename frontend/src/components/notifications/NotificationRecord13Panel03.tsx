...
export interface NotificationRecord13Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * NotificationRecord13Panel03 — operational panel for notifications domain slice 13/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function NotificationRecord13Panel03({ facilityId, title = 'NotificationRecord13 Workspace', onSelect }: NotificationRecord13Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--notifications" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} notifications records (slice 13-03)
      </footer>
    </section>
  );
}

export default NotificationRecord13Panel03;
