...
export interface HospitalRecord04Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord04Panel01 — operational panel for hospitals domain slice 04/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord04Panel01({ facilityId, title = 'HospitalRecord04 Workspace', onSelect }: HospitalRecord04Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 04-01)
      </footer>
    </section>
  );
}

export default HospitalRecord04Panel01;
