...
export interface HospitalRecord22Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord22Panel01 — operational panel for hospitals domain slice 22/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord22Panel01({ facilityId, title = 'HospitalRecord22 Workspace', onSelect }: HospitalRecord22Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 22-01)
      </footer>
    </section>
  );
}

export default HospitalRecord22Panel01;
