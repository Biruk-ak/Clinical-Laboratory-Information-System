...
export interface HospitalRecord13Panel04Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord13Panel04 — operational panel for hospitals domain slice 13/04.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord13Panel04({ facilityId, title = 'HospitalRecord13 Workspace', onSelect }: HospitalRecord13Panel04Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 13-04)
      </footer>
    </section>
  );
}

export default HospitalRecord13Panel04;
