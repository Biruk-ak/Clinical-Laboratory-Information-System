...
export interface HospitalRecord22Panel06Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord22Panel06 — operational panel for hospitals domain slice 22/06.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord22Panel06({ facilityId, title = 'HospitalRecord22 Workspace', onSelect }: HospitalRecord22Panel06Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 22-06)
      </footer>
    </section>
  );
}

export default HospitalRecord22Panel06;
