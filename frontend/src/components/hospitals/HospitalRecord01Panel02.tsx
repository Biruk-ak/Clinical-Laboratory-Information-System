...
export interface HospitalRecord01Panel02Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord01Panel02 — operational panel for hospitals domain slice 01/02.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord01Panel02({ facilityId, title = 'HospitalRecord01 Workspace', onSelect }: HospitalRecord01Panel02Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 01-02)
      </footer>
    </section>
  );
}

export default HospitalRecord01Panel02;
