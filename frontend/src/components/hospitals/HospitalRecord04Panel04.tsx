...
export interface HospitalRecord04Panel04Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord04Panel04 — operational panel for hospitals domain slice 04/04.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord04Panel04({ facilityId, title = 'HospitalRecord04 Workspace', onSelect }: HospitalRecord04Panel04Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 04-04)
      </footer>
    </section>
  );
}

export default HospitalRecord04Panel04;
