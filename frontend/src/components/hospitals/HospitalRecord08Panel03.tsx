...
export interface HospitalRecord08Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord08Panel03 — operational panel for hospitals domain slice 08/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord08Panel03({ facilityId, title = 'HospitalRecord08 Workspace', onSelect }: HospitalRecord08Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 08-03)
      </footer>
    </section>
  );
}

export default HospitalRecord08Panel03;
