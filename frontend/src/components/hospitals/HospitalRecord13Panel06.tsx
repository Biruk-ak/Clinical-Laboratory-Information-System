...
export interface HospitalRecord13Panel06Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord13Panel06 — operational panel for hospitals domain slice 13/06.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord13Panel06({ facilityId, title = 'HospitalRecord13 Workspace', onSelect }: HospitalRecord13Panel06Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 13-06)
      </footer>
    </section>
  );
}

export default HospitalRecord13Panel06;
