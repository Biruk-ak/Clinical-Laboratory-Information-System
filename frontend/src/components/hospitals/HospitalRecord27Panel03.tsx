...
export interface HospitalRecord27Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord27Panel03 — operational panel for hospitals domain slice 27/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord27Panel03({ facilityId, title = 'HospitalRecord27 Workspace', onSelect }: HospitalRecord27Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 27-03)
      </footer>
    </section>
  );
}

export default HospitalRecord27Panel03;
