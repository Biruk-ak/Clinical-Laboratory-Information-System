...
export interface HospitalRecord10Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord10Panel01 — operational panel for hospitals domain slice 10/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord10Panel01({ facilityId, title = 'HospitalRecord10 Workspace', onSelect }: HospitalRecord10Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 10-01)
      </footer>
    </section>
  );
}

export default HospitalRecord10Panel01;
