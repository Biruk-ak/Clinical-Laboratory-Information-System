...
export interface HospitalRecord10Panel04Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord10Panel04 — operational panel for hospitals domain slice 10/04.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord10Panel04({ facilityId, title = 'HospitalRecord10 Workspace', onSelect }: HospitalRecord10Panel04Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 10-04)
      </footer>
    </section>
  );
}

export default HospitalRecord10Panel04;
