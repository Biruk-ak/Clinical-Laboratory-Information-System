...
export interface HospitalRecord24Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord24Panel01 — operational panel for hospitals domain slice 24/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord24Panel01({ facilityId, title = 'HospitalRecord24 Workspace', onSelect }: HospitalRecord24Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 24-01)
      </footer>
    </section>
  );
}

export default HospitalRecord24Panel01;
