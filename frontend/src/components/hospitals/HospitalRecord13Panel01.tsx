...
export interface HospitalRecord13Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord13Panel01 — operational panel for hospitals domain slice 13/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord13Panel01({ facilityId, title = 'HospitalRecord13 Workspace', onSelect }: HospitalRecord13Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 13-01)
      </footer>
    </section>
  );
}

export default HospitalRecord13Panel01;
