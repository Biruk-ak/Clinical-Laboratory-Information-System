...
export interface HospitalRecord10Panel05Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord10Panel05 — operational panel for hospitals domain slice 10/05.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord10Panel05({ facilityId, title = 'HospitalRecord10 Workspace', onSelect }: HospitalRecord10Panel05Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 10-05)
      </footer>
    </section>
  );
}

export default HospitalRecord10Panel05;
