...
export interface HospitalRecord24Panel04Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord24Panel04 — operational panel for hospitals domain slice 24/04.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord24Panel04({ facilityId, title = 'HospitalRecord24 Workspace', onSelect }: HospitalRecord24Panel04Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 24-04)
      </footer>
    </section>
  );
}

export default HospitalRecord24Panel04;
