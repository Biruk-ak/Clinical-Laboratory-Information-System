...
export interface HospitalRecord22Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord22Panel03 — operational panel for hospitals domain slice 22/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord22Panel03({ facilityId, title = 'HospitalRecord22 Workspace', onSelect }: HospitalRecord22Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 22-03)
      </footer>
    </section>
  );
}

export default HospitalRecord22Panel03;
