...
export interface HospitalRecord07Panel02Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord07Panel02 — operational panel for hospitals domain slice 07/02.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord07Panel02({ facilityId, title = 'HospitalRecord07 Workspace', onSelect }: HospitalRecord07Panel02Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 07-02)
      </footer>
    </section>
  );
}

export default HospitalRecord07Panel02;
