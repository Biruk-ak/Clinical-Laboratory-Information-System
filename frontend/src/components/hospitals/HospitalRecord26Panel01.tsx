...
export interface HospitalRecord26Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord26Panel01 — operational panel for hospitals domain slice 26/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord26Panel01({ facilityId, title = 'HospitalRecord26 Workspace', onSelect }: HospitalRecord26Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 26-01)
      </footer>
    </section>
  );
}

export default HospitalRecord26Panel01;
