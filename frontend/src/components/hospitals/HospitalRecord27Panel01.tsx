...
export interface HospitalRecord27Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord27Panel01 — operational panel for hospitals domain slice 27/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord27Panel01({ facilityId, title = 'HospitalRecord27 Workspace', onSelect }: HospitalRecord27Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 27-01)
      </footer>
    </section>
  );
}

export default HospitalRecord27Panel01;
