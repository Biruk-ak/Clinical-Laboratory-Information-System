...
export interface HospitalRecord13Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord13Panel03 — operational panel for hospitals domain slice 13/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord13Panel03({ facilityId, title = 'HospitalRecord13 Workspace', onSelect }: HospitalRecord13Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 13-03)
      </footer>
    </section>
  );
}

export default HospitalRecord13Panel03;
