...
export interface HospitalRecord13Panel05Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord13Panel05 — operational panel for hospitals domain slice 13/05.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord13Panel05({ facilityId, title = 'HospitalRecord13 Workspace', onSelect }: HospitalRecord13Panel05Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 13-05)
      </footer>
    </section>
  );
}

export default HospitalRecord13Panel05;
