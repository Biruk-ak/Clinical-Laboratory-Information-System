...
export interface HospitalRecord22Panel05Props {
  facilityId: string;
  title?: string;
...
/**
 * HospitalRecord22Panel05 — operational panel for hospitals domain slice 22/05.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function HospitalRecord22Panel05({ facilityId, title = 'HospitalRecord22 Workspace', onSelect }: HospitalRecord22Panel05Props) {
  ...
  return (
    <section className="lis-panel lis-panel--hospitals" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} hospitals records (slice 22-05)
      </footer>
    </section>
  );
}

export default HospitalRecord22Panel05;
