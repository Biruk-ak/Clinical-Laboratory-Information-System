...
export interface AnalyticsRecord01Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * AnalyticsRecord01Panel01 — operational panel for analytics domain slice 01/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AnalyticsRecord01Panel01({ facilityId, title = 'AnalyticsRecord01 Workspace', onSelect }: AnalyticsRecord01Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--analytics" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} analytics records (slice 01-01)
      </footer>
    </section>
  );
}

export default AnalyticsRecord01Panel01;
