...
export interface AuthRecord11Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord11Panel01 — operational panel for auth domain slice 11/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord11Panel01({ facilityId, title = 'AuthRecord11 Workspace', onSelect }: AuthRecord11Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 11-01)
      </footer>
    </section>
  );
}

export default AuthRecord11Panel01;
