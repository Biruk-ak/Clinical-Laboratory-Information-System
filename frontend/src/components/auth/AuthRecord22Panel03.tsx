...
export interface AuthRecord22Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord22Panel03 — operational panel for auth domain slice 22/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord22Panel03({ facilityId, title = 'AuthRecord22 Workspace', onSelect }: AuthRecord22Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 22-03)
      </footer>
    </section>
  );
}

export default AuthRecord22Panel03;
