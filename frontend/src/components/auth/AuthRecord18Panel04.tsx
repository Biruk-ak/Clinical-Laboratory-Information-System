...
export interface AuthRecord18Panel04Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord18Panel04 — operational panel for auth domain slice 18/04.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord18Panel04({ facilityId, title = 'AuthRecord18 Workspace', onSelect }: AuthRecord18Panel04Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 18-04)
      </footer>
    </section>
  );
}

export default AuthRecord18Panel04;
