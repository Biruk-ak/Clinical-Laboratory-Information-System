...
export interface AuthRecord14Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord14Panel01 — operational panel for auth domain slice 14/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord14Panel01({ facilityId, title = 'AuthRecord14 Workspace', onSelect }: AuthRecord14Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 14-01)
      </footer>
    </section>
  );
}

export default AuthRecord14Panel01;
