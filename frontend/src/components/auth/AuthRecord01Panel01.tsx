...
export interface AuthRecord01Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord01Panel01 — operational panel for auth domain slice 01/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord01Panel01({ facilityId, title = 'AuthRecord01 Workspace', onSelect }: AuthRecord01Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 01-01)
      </footer>
    </section>
  );
}

export default AuthRecord01Panel01;
