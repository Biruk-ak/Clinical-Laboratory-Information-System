...
export interface AuthRecord02Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord02Panel03 — operational panel for auth domain slice 02/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord02Panel03({ facilityId, title = 'AuthRecord02 Workspace', onSelect }: AuthRecord02Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 02-03)
      </footer>
    </section>
  );
}

export default AuthRecord02Panel03;
