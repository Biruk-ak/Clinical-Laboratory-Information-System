...
export interface AuthRecord05Panel06Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord05Panel06 — operational panel for auth domain slice 05/06.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord05Panel06({ facilityId, title = 'AuthRecord05 Workspace', onSelect }: AuthRecord05Panel06Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 05-06)
      </footer>
    </section>
  );
}

export default AuthRecord05Panel06;
