...
export interface AuthRecord06Panel02Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord06Panel02 — operational panel for auth domain slice 06/02.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord06Panel02({ facilityId, title = 'AuthRecord06 Workspace', onSelect }: AuthRecord06Panel02Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 06-02)
      </footer>
    </section>
  );
}

export default AuthRecord06Panel02;
