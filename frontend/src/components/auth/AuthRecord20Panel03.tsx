...
export interface AuthRecord20Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord20Panel03 — operational panel for auth domain slice 20/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord20Panel03({ facilityId, title = 'AuthRecord20 Workspace', onSelect }: AuthRecord20Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 20-03)
      </footer>
    </section>
  );
}

export default AuthRecord20Panel03;
