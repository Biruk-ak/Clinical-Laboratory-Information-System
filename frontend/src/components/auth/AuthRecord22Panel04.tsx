...
export interface AuthRecord22Panel04Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord22Panel04 — operational panel for auth domain slice 22/04.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord22Panel04({ facilityId, title = 'AuthRecord22 Workspace', onSelect }: AuthRecord22Panel04Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 22-04)
      </footer>
    </section>
  );
}

export default AuthRecord22Panel04;
