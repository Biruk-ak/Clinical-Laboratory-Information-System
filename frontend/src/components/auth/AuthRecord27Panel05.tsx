...
export interface AuthRecord27Panel05Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord27Panel05 — operational panel for auth domain slice 27/05.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord27Panel05({ facilityId, title = 'AuthRecord27 Workspace', onSelect }: AuthRecord27Panel05Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 27-05)
      </footer>
    </section>
  );
}

export default AuthRecord27Panel05;
