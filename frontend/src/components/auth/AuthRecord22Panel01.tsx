...
export interface AuthRecord22Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord22Panel01 — operational panel for auth domain slice 22/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord22Panel01({ facilityId, title = 'AuthRecord22 Workspace', onSelect }: AuthRecord22Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 22-01)
      </footer>
    </section>
  );
}

export default AuthRecord22Panel01;
