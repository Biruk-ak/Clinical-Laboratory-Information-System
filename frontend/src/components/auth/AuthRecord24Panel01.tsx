...
export interface AuthRecord24Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord24Panel01 — operational panel for auth domain slice 24/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord24Panel01({ facilityId, title = 'AuthRecord24 Workspace', onSelect }: AuthRecord24Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 24-01)
      </footer>
    </section>
  );
}

export default AuthRecord24Panel01;
