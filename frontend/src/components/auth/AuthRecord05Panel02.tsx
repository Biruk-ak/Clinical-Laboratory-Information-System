...
export interface AuthRecord05Panel02Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord05Panel02 — operational panel for auth domain slice 05/02.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord05Panel02({ facilityId, title = 'AuthRecord05 Workspace', onSelect }: AuthRecord05Panel02Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 05-02)
      </footer>
    </section>
  );
}

export default AuthRecord05Panel02;
