...
export interface AuthRecord17Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord17Panel01 — operational panel for auth domain slice 17/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord17Panel01({ facilityId, title = 'AuthRecord17 Workspace', onSelect }: AuthRecord17Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 17-01)
      </footer>
    </section>
  );
}

export default AuthRecord17Panel01;
