...
export interface AuthRecord20Panel05Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord20Panel05 — operational panel for auth domain slice 20/05.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord20Panel05({ facilityId, title = 'AuthRecord20 Workspace', onSelect }: AuthRecord20Panel05Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 20-05)
      </footer>
    </section>
  );
}

export default AuthRecord20Panel05;
