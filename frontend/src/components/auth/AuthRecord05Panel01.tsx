...
export interface AuthRecord05Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord05Panel01 — operational panel for auth domain slice 05/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord05Panel01({ facilityId, title = 'AuthRecord05 Workspace', onSelect }: AuthRecord05Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 05-01)
      </footer>
    </section>
  );
}

export default AuthRecord05Panel01;
