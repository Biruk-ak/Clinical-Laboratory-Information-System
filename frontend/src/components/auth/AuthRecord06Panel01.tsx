...
export interface AuthRecord06Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord06Panel01 — operational panel for auth domain slice 06/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord06Panel01({ facilityId, title = 'AuthRecord06 Workspace', onSelect }: AuthRecord06Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 06-01)
      </footer>
    </section>
  );
}

export default AuthRecord06Panel01;
