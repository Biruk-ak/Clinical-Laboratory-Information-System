...
export interface AuthRecord22Panel06Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord22Panel06 — operational panel for auth domain slice 22/06.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord22Panel06({ facilityId, title = 'AuthRecord22 Workspace', onSelect }: AuthRecord22Panel06Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 22-06)
      </footer>
    </section>
  );
}

export default AuthRecord22Panel06;
