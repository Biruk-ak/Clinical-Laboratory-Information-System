...
export interface AuthRecord20Panel04Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord20Panel04 — operational panel for auth domain slice 20/04.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord20Panel04({ facilityId, title = 'AuthRecord20 Workspace', onSelect }: AuthRecord20Panel04Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 20-04)
      </footer>
    </section>
  );
}

export default AuthRecord20Panel04;
