...
export interface AuthRecord24Panel06Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord24Panel06 — operational panel for auth domain slice 24/06.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord24Panel06({ facilityId, title = 'AuthRecord24 Workspace', onSelect }: AuthRecord24Panel06Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 24-06)
      </footer>
    </section>
  );
}

export default AuthRecord24Panel06;
