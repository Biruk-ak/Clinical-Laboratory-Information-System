...
export interface AuthRecord25Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord25Panel01 — operational panel for auth domain slice 25/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord25Panel01({ facilityId, title = 'AuthRecord25 Workspace', onSelect }: AuthRecord25Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 25-01)
      </footer>
    </section>
  );
}

export default AuthRecord25Panel01;
