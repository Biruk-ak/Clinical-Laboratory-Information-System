...
export interface AuthRecord17Panel05Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord17Panel05 — operational panel for auth domain slice 17/05.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord17Panel05({ facilityId, title = 'AuthRecord17 Workspace', onSelect }: AuthRecord17Panel05Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 17-05)
      </footer>
    </section>
  );
}

export default AuthRecord17Panel05;
