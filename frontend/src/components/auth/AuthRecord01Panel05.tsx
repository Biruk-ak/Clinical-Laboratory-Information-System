...
export interface AuthRecord01Panel05Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord01Panel05 — operational panel for auth domain slice 01/05.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord01Panel05({ facilityId, title = 'AuthRecord01 Workspace', onSelect }: AuthRecord01Panel05Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 01-05)
      </footer>
    </section>
  );
}

export default AuthRecord01Panel05;
