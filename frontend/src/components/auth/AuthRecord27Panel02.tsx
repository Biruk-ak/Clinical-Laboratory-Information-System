...
export interface AuthRecord27Panel02Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord27Panel02 — operational panel for auth domain slice 27/02.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord27Panel02({ facilityId, title = 'AuthRecord27 Workspace', onSelect }: AuthRecord27Panel02Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 27-02)
      </footer>
    </section>
  );
}

export default AuthRecord27Panel02;
