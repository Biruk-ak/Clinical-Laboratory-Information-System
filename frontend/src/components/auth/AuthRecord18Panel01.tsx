...
export interface AuthRecord18Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord18Panel01 — operational panel for auth domain slice 18/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord18Panel01({ facilityId, title = 'AuthRecord18 Workspace', onSelect }: AuthRecord18Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 18-01)
      </footer>
    </section>
  );
}

export default AuthRecord18Panel01;
