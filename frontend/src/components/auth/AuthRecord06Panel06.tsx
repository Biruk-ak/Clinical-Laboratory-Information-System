...
export interface AuthRecord06Panel06Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord06Panel06 — operational panel for auth domain slice 06/06.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord06Panel06({ facilityId, title = 'AuthRecord06 Workspace', onSelect }: AuthRecord06Panel06Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 06-06)
      </footer>
    </section>
  );
}

export default AuthRecord06Panel06;
