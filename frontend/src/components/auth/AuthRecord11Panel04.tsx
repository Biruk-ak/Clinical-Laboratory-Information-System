...
export interface AuthRecord11Panel04Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord11Panel04 — operational panel for auth domain slice 11/04.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord11Panel04({ facilityId, title = 'AuthRecord11 Workspace', onSelect }: AuthRecord11Panel04Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 11-04)
      </footer>
    </section>
  );
}

export default AuthRecord11Panel04;
