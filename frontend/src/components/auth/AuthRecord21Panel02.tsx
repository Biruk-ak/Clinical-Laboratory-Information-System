...
export interface AuthRecord21Panel02Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord21Panel02 — operational panel for auth domain slice 21/02.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord21Panel02({ facilityId, title = 'AuthRecord21 Workspace', onSelect }: AuthRecord21Panel02Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 21-02)
      </footer>
    </section>
  );
}

export default AuthRecord21Panel02;
