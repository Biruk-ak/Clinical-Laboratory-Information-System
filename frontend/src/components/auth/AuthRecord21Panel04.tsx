...
export interface AuthRecord21Panel04Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord21Panel04 — operational panel for auth domain slice 21/04.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord21Panel04({ facilityId, title = 'AuthRecord21 Workspace', onSelect }: AuthRecord21Panel04Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 21-04)
      </footer>
    </section>
  );
}

export default AuthRecord21Panel04;
