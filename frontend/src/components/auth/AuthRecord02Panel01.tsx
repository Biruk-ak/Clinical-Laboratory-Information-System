...
export interface AuthRecord02Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord02Panel01 — operational panel for auth domain slice 02/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord02Panel01({ facilityId, title = 'AuthRecord02 Workspace', onSelect }: AuthRecord02Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 02-01)
      </footer>
    </section>
  );
}

export default AuthRecord02Panel01;
