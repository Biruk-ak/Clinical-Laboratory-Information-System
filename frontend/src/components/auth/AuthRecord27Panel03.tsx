...
export interface AuthRecord27Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord27Panel03 — operational panel for auth domain slice 27/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord27Panel03({ facilityId, title = 'AuthRecord27 Workspace', onSelect }: AuthRecord27Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 27-03)
      </footer>
    </section>
  );
}

export default AuthRecord27Panel03;
