...
export interface AuthRecord21Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord21Panel01 — operational panel for auth domain slice 21/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord21Panel01({ facilityId, title = 'AuthRecord21 Workspace', onSelect }: AuthRecord21Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 21-01)
      </footer>
    </section>
  );
}

export default AuthRecord21Panel01;
