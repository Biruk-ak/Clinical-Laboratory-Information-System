...
export interface AuthRecord24Panel04Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord24Panel04 — operational panel for auth domain slice 24/04.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord24Panel04({ facilityId, title = 'AuthRecord24 Workspace', onSelect }: AuthRecord24Panel04Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 24-04)
      </footer>
    </section>
  );
}

export default AuthRecord24Panel04;
