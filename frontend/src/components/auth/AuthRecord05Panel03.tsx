...
export interface AuthRecord05Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord05Panel03 — operational panel for auth domain slice 05/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord05Panel03({ facilityId, title = 'AuthRecord05 Workspace', onSelect }: AuthRecord05Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 05-03)
      </footer>
    </section>
  );
}

export default AuthRecord05Panel03;
