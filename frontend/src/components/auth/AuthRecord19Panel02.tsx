...
export interface AuthRecord19Panel02Props {
  facilityId: string;
  title?: string;
...
/**
 * AuthRecord19Panel02 — operational panel for auth domain slice 19/02.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function AuthRecord19Panel02({ facilityId, title = 'AuthRecord19 Workspace', onSelect }: AuthRecord19Panel02Props) {
  ...
  return (
    <section className="lis-panel lis-panel--auth" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} auth records (slice 19-02)
      </footer>
    </section>
  );
}

export default AuthRecord19Panel02;
