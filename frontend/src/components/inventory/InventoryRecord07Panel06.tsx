...
export interface InventoryRecord07Panel06Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord07Panel06 — operational panel for inventory domain slice 07/06.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord07Panel06({ facilityId, title = 'InventoryRecord07 Workspace', onSelect }: InventoryRecord07Panel06Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 07-06)
      </footer>
    </section>
  );
}

export default InventoryRecord07Panel06;
