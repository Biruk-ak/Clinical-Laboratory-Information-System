...
export interface InventoryRecord10Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord10Panel03 — operational panel for inventory domain slice 10/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord10Panel03({ facilityId, title = 'InventoryRecord10 Workspace', onSelect }: InventoryRecord10Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 10-03)
      </footer>
    </section>
  );
}

export default InventoryRecord10Panel03;
