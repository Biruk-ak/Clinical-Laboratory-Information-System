...
export interface InventoryRecord23Panel04Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord23Panel04 — operational panel for inventory domain slice 23/04.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord23Panel04({ facilityId, title = 'InventoryRecord23 Workspace', onSelect }: InventoryRecord23Panel04Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 23-04)
      </footer>
    </section>
  );
}

export default InventoryRecord23Panel04;
