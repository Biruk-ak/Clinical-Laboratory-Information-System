...
export interface InventoryRecord26Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord26Panel03 — operational panel for inventory domain slice 26/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord26Panel03({ facilityId, title = 'InventoryRecord26 Workspace', onSelect }: InventoryRecord26Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 26-03)
      </footer>
    </section>
  );
}

export default InventoryRecord26Panel03;
