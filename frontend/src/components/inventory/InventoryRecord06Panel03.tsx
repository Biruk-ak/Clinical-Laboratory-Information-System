...
export interface InventoryRecord06Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord06Panel03 — operational panel for inventory domain slice 06/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord06Panel03({ facilityId, title = 'InventoryRecord06 Workspace', onSelect }: InventoryRecord06Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 06-03)
      </footer>
    </section>
  );
}

export default InventoryRecord06Panel03;
