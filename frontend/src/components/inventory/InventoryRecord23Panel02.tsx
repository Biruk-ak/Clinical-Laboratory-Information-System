...
export interface InventoryRecord23Panel02Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord23Panel02 — operational panel for inventory domain slice 23/02.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord23Panel02({ facilityId, title = 'InventoryRecord23 Workspace', onSelect }: InventoryRecord23Panel02Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 23-02)
      </footer>
    </section>
  );
}

export default InventoryRecord23Panel02;
