...
export interface InventoryRecord11Panel06Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord11Panel06 — operational panel for inventory domain slice 11/06.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord11Panel06({ facilityId, title = 'InventoryRecord11 Workspace', onSelect }: InventoryRecord11Panel06Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 11-06)
      </footer>
    </section>
  );
}

export default InventoryRecord11Panel06;
