...
export interface InventoryRecord20Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord20Panel01 — operational panel for inventory domain slice 20/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord20Panel01({ facilityId, title = 'InventoryRecord20 Workspace', onSelect }: InventoryRecord20Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 20-01)
      </footer>
    </section>
  );
}

export default InventoryRecord20Panel01;
