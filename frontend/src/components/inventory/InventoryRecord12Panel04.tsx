...
export interface InventoryRecord12Panel04Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord12Panel04 — operational panel for inventory domain slice 12/04.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord12Panel04({ facilityId, title = 'InventoryRecord12 Workspace', onSelect }: InventoryRecord12Panel04Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 12-04)
      </footer>
    </section>
  );
}

export default InventoryRecord12Panel04;
