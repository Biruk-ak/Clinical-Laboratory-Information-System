...
export interface InventoryRecord03Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord03Panel03 — operational panel for inventory domain slice 03/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord03Panel03({ facilityId, title = 'InventoryRecord03 Workspace', onSelect }: InventoryRecord03Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 03-03)
      </footer>
    </section>
  );
}

export default InventoryRecord03Panel03;
