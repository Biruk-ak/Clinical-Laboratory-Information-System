...
export interface InventoryRecord27Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord27Panel03 — operational panel for inventory domain slice 27/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord27Panel03({ facilityId, title = 'InventoryRecord27 Workspace', onSelect }: InventoryRecord27Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 27-03)
      </footer>
    </section>
  );
}

export default InventoryRecord27Panel03;
