...
export interface InventoryRecord18Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord18Panel01 — operational panel for inventory domain slice 18/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord18Panel01({ facilityId, title = 'InventoryRecord18 Workspace', onSelect }: InventoryRecord18Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 18-01)
      </footer>
    </section>
  );
}

export default InventoryRecord18Panel01;
