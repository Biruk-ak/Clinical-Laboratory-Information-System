...
export interface InventoryRecord27Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord27Panel01 — operational panel for inventory domain slice 27/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord27Panel01({ facilityId, title = 'InventoryRecord27 Workspace', onSelect }: InventoryRecord27Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 27-01)
      </footer>
    </section>
  );
}

export default InventoryRecord27Panel01;
