...
export interface InventoryRecord17Panel05Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord17Panel05 — operational panel for inventory domain slice 17/05.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord17Panel05({ facilityId, title = 'InventoryRecord17 Workspace', onSelect }: InventoryRecord17Panel05Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 17-05)
      </footer>
    </section>
  );
}

export default InventoryRecord17Panel05;
