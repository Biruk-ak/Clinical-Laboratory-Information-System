...
export interface InventoryRecord12Panel02Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord12Panel02 — operational panel for inventory domain slice 12/02.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord12Panel02({ facilityId, title = 'InventoryRecord12 Workspace', onSelect }: InventoryRecord12Panel02Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 12-02)
      </footer>
    </section>
  );
}

export default InventoryRecord12Panel02;
