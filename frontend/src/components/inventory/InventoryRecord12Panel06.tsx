...
export interface InventoryRecord12Panel06Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord12Panel06 — operational panel for inventory domain slice 12/06.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord12Panel06({ facilityId, title = 'InventoryRecord12 Workspace', onSelect }: InventoryRecord12Panel06Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 12-06)
      </footer>
    </section>
  );
}

export default InventoryRecord12Panel06;
