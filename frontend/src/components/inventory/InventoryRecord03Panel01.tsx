...
export interface InventoryRecord03Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord03Panel01 — operational panel for inventory domain slice 03/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord03Panel01({ facilityId, title = 'InventoryRecord03 Workspace', onSelect }: InventoryRecord03Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 03-01)
      </footer>
    </section>
  );
}

export default InventoryRecord03Panel01;
