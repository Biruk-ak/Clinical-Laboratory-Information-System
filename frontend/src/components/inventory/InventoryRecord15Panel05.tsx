...
export interface InventoryRecord15Panel05Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord15Panel05 — operational panel for inventory domain slice 15/05.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord15Panel05({ facilityId, title = 'InventoryRecord15 Workspace', onSelect }: InventoryRecord15Panel05Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 15-05)
      </footer>
    </section>
  );
}

export default InventoryRecord15Panel05;
