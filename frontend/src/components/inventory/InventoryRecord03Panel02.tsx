...
export interface InventoryRecord03Panel02Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord03Panel02 — operational panel for inventory domain slice 03/02.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord03Panel02({ facilityId, title = 'InventoryRecord03 Workspace', onSelect }: InventoryRecord03Panel02Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 03-02)
      </footer>
    </section>
  );
}

export default InventoryRecord03Panel02;
