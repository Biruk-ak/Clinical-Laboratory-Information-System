...
export interface InventoryRecord13Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord13Panel03 — operational panel for inventory domain slice 13/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord13Panel03({ facilityId, title = 'InventoryRecord13 Workspace', onSelect }: InventoryRecord13Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 13-03)
      </footer>
    </section>
  );
}

export default InventoryRecord13Panel03;
