...
export interface InventoryRecord07Panel04Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord07Panel04 — operational panel for inventory domain slice 07/04.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord07Panel04({ facilityId, title = 'InventoryRecord07 Workspace', onSelect }: InventoryRecord07Panel04Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 07-04)
      </footer>
    </section>
  );
}

export default InventoryRecord07Panel04;
