...
export interface InventoryRecord02Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord02Panel03 — operational panel for inventory domain slice 02/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord02Panel03({ facilityId, title = 'InventoryRecord02 Workspace', onSelect }: InventoryRecord02Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 02-03)
      </footer>
    </section>
  );
}

export default InventoryRecord02Panel03;
