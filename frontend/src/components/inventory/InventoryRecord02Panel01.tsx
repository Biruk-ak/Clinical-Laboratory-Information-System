...
export interface InventoryRecord02Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord02Panel01 — operational panel for inventory domain slice 02/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord02Panel01({ facilityId, title = 'InventoryRecord02 Workspace', onSelect }: InventoryRecord02Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 02-01)
      </footer>
    </section>
  );
}

export default InventoryRecord02Panel01;
