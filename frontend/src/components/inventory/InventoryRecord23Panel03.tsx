...
export interface InventoryRecord23Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord23Panel03 — operational panel for inventory domain slice 23/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord23Panel03({ facilityId, title = 'InventoryRecord23 Workspace', onSelect }: InventoryRecord23Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 23-03)
      </footer>
    </section>
  );
}

export default InventoryRecord23Panel03;
