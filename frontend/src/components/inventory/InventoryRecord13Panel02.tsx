...
export interface InventoryRecord13Panel02Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord13Panel02 — operational panel for inventory domain slice 13/02.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord13Panel02({ facilityId, title = 'InventoryRecord13 Workspace', onSelect }: InventoryRecord13Panel02Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 13-02)
      </footer>
    </section>
  );
}

export default InventoryRecord13Panel02;
