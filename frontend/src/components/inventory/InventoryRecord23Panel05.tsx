...
export interface InventoryRecord23Panel05Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord23Panel05 — operational panel for inventory domain slice 23/05.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord23Panel05({ facilityId, title = 'InventoryRecord23 Workspace', onSelect }: InventoryRecord23Panel05Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 23-05)
      </footer>
    </section>
  );
}

export default InventoryRecord23Panel05;
