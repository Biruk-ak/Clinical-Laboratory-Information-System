...
export interface InventoryRecord07Panel03Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord07Panel03 — operational panel for inventory domain slice 07/03.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord07Panel03({ facilityId, title = 'InventoryRecord07 Workspace', onSelect }: InventoryRecord07Panel03Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 07-03)
      </footer>
    </section>
  );
}

export default InventoryRecord07Panel03;
