...
export interface InventoryRecord10Panel04Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord10Panel04 — operational panel for inventory domain slice 10/04.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord10Panel04({ facilityId, title = 'InventoryRecord10 Workspace', onSelect }: InventoryRecord10Panel04Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 10-04)
      </footer>
    </section>
  );
}

export default InventoryRecord10Panel04;
