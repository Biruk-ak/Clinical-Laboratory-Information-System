...
export interface InventoryRecord26Panel05Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord26Panel05 — operational panel for inventory domain slice 26/05.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord26Panel05({ facilityId, title = 'InventoryRecord26 Workspace', onSelect }: InventoryRecord26Panel05Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 26-05)
      </footer>
    </section>
  );
}

export default InventoryRecord26Panel05;
