...
export interface InventoryRecord07Panel02Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord07Panel02 — operational panel for inventory domain slice 07/02.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord07Panel02({ facilityId, title = 'InventoryRecord07 Workspace', onSelect }: InventoryRecord07Panel02Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 07-02)
      </footer>
    </section>
  );
}

export default InventoryRecord07Panel02;
