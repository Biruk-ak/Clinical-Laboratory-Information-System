...
export interface InventoryRecord18Panel04Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord18Panel04 — operational panel for inventory domain slice 18/04.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord18Panel04({ facilityId, title = 'InventoryRecord18 Workspace', onSelect }: InventoryRecord18Panel04Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 18-04)
      </footer>
    </section>
  );
}

export default InventoryRecord18Panel04;
