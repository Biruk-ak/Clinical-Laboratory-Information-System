...
export interface InventoryRecord20Panel06Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord20Panel06 — operational panel for inventory domain slice 20/06.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord20Panel06({ facilityId, title = 'InventoryRecord20 Workspace', onSelect }: InventoryRecord20Panel06Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 20-06)
      </footer>
    </section>
  );
}

export default InventoryRecord20Panel06;
