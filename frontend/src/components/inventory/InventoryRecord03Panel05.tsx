...
export interface InventoryRecord03Panel05Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord03Panel05 — operational panel for inventory domain slice 03/05.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord03Panel05({ facilityId, title = 'InventoryRecord03 Workspace', onSelect }: InventoryRecord03Panel05Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 03-05)
      </footer>
    </section>
  );
}

export default InventoryRecord03Panel05;
