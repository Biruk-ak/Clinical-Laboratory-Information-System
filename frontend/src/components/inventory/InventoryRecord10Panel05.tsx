...
export interface InventoryRecord10Panel05Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord10Panel05 — operational panel for inventory domain slice 10/05.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord10Panel05({ facilityId, title = 'InventoryRecord10 Workspace', onSelect }: InventoryRecord10Panel05Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 10-05)
      </footer>
    </section>
  );
}

export default InventoryRecord10Panel05;
