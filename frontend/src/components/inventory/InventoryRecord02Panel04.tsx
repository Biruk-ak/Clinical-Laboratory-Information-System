...
export interface InventoryRecord02Panel04Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord02Panel04 — operational panel for inventory domain slice 02/04.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord02Panel04({ facilityId, title = 'InventoryRecord02 Workspace', onSelect }: InventoryRecord02Panel04Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 02-04)
      </footer>
    </section>
  );
}

export default InventoryRecord02Panel04;
