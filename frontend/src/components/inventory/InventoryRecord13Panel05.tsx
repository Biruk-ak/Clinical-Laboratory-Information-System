...
export interface InventoryRecord13Panel05Props {
  facilityId: string;
  title?: string;
...
/**
 * InventoryRecord13Panel05 — operational panel for inventory domain slice 13/05.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function InventoryRecord13Panel05({ facilityId, title = 'InventoryRecord13 Workspace', onSelect }: InventoryRecord13Panel05Props) {
  ...
  return (
    <section className="lis-panel lis-panel--inventory" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} inventory records (slice 13-05)
      </footer>
    </section>
  );
}

export default InventoryRecord13Panel05;
