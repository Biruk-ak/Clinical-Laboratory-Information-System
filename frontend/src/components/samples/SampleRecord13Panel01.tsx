...
export interface SampleRecord13Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * SampleRecord13Panel01 — operational panel for samples domain slice 13/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function SampleRecord13Panel01({ facilityId, title = 'SampleRecord13 Workspace', onSelect }: SampleRecord13Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--samples" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} samples records (slice 13-01)
      </footer>
    </section>
  );
}

export default SampleRecord13Panel01;
