...
export interface SampleRecord10Panel02Props {
  facilityId: string;
  title?: string;
...
/**
 * SampleRecord10Panel02 — operational panel for samples domain slice 10/02.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function SampleRecord10Panel02({ facilityId, title = 'SampleRecord10 Workspace', onSelect }: SampleRecord10Panel02Props) {
  ...
  return (
    <section className="lis-panel lis-panel--samples" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} samples records (slice 10-02)
      </footer>
    </section>
  );
}

export default SampleRecord10Panel02;
