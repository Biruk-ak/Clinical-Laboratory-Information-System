...
export interface SampleRecord26Panel02Props {
  facilityId: string;
  title?: string;
...
/**
 * SampleRecord26Panel02 — operational panel for samples domain slice 26/02.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function SampleRecord26Panel02({ facilityId, title = 'SampleRecord26 Workspace', onSelect }: SampleRecord26Panel02Props) {
  ...
  return (
    <section className="lis-panel lis-panel--samples" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} samples records (slice 26-02)
      </footer>
    </section>
  );
}

export default SampleRecord26Panel02;
