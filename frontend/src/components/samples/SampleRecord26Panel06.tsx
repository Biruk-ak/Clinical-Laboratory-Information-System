...
export interface SampleRecord26Panel06Props {
  facilityId: string;
  title?: string;
...
/**
 * SampleRecord26Panel06 — operational panel for samples domain slice 26/06.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function SampleRecord26Panel06({ facilityId, title = 'SampleRecord26 Workspace', onSelect }: SampleRecord26Panel06Props) {
  ...
  return (
    <section className="lis-panel lis-panel--samples" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} samples records (slice 26-06)
      </footer>
    </section>
  );
}

export default SampleRecord26Panel06;
