...
export interface SampleRecord21Panel01Props {
  facilityId: string;
  title?: string;
...
/**
 * SampleRecord21Panel01 — operational panel for samples domain slice 21/01.
 * Supports list, filter by active status, and soft-archive actions.
 */
export function SampleRecord21Panel01({ facilityId, title = 'SampleRecord21 Workspace', onSelect }: SampleRecord21Panel01Props) {
  ...
  return (
    <section className="lis-panel lis-panel--samples" aria-label={title}>
      ...
      <footer className="lis-panel__footer">
        Showing {filtered.length} of {items.length} samples records (slice 21-01)
      </footer>
    </section>
  );
}

export default SampleRecord21Panel01;
