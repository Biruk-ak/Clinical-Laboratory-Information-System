...
// AuthRecord28 represents a domain record in the auth bounded context.
type AuthRecord28 struct {
...
// Validate performs lightweight domain validation for AuthRecord28.
func (r *AuthRecord28) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *AuthRecord28) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *AuthRecord28) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
