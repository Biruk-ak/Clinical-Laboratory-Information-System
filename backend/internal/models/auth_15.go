...
// AuthRecord15 represents a domain record in the auth bounded context.
type AuthRecord15 struct {
...
// Validate performs lightweight domain validation for AuthRecord15.
func (r *AuthRecord15) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *AuthRecord15) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *AuthRecord15) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
