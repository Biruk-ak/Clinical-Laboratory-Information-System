...
// InventoryRecord16 represents a domain record in the inventory bounded context.
type InventoryRecord16 struct {
...
// Validate performs lightweight domain validation for InventoryRecord16.
func (r *InventoryRecord16) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *InventoryRecord16) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *InventoryRecord16) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
