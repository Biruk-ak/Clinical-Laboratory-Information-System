...
// ResultRecord17 represents a domain record in the results bounded context.
type ResultRecord17 struct {
...
// Validate performs lightweight domain validation for ResultRecord17.
func (r *ResultRecord17) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *ResultRecord17) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *ResultRecord17) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
