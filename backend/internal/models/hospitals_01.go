...
// HospitalRecord01 represents a domain record in the hospitals bounded context.
type HospitalRecord01 struct {
...
// Validate performs lightweight domain validation for HospitalRecord01.
func (r *HospitalRecord01) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *HospitalRecord01) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *HospitalRecord01) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
