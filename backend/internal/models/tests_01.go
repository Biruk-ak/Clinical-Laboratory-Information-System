...
// LabTestRecord01 represents a domain record in the tests bounded context.
type LabTestRecord01 struct {
...
// Validate performs lightweight domain validation for LabTestRecord01.
func (r *LabTestRecord01) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *LabTestRecord01) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *LabTestRecord01) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
