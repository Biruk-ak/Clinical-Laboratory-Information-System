...
// LabTestRecord03 represents a domain record in the tests bounded context.
type LabTestRecord03 struct {
...
// Validate performs lightweight domain validation for LabTestRecord03.
func (r *LabTestRecord03) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *LabTestRecord03) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *LabTestRecord03) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
