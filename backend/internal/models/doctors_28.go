...
// DoctorRecord28 represents a domain record in the doctors bounded context.
type DoctorRecord28 struct {
...
// Validate performs lightweight domain validation for DoctorRecord28.
func (r *DoctorRecord28) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *DoctorRecord28) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *DoctorRecord28) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
