...
// ReportRecord23 represents a domain record in the reports bounded context.
type ReportRecord23 struct {
...
// Validate performs lightweight domain validation for ReportRecord23.
func (r *ReportRecord23) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *ReportRecord23) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *ReportRecord23) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
