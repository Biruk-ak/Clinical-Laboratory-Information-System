...
// AnalyticsRecord21 represents a domain record in the analytics bounded context.
type AnalyticsRecord21 struct {
...
// Validate performs lightweight domain validation for AnalyticsRecord21.
func (r *AnalyticsRecord21) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *AnalyticsRecord21) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *AnalyticsRecord21) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
