...
// SampleRecord20 represents a domain record in the samples bounded context.
type SampleRecord20 struct {
...
// Validate performs lightweight domain validation for SampleRecord20.
func (r *SampleRecord20) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *SampleRecord20) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *SampleRecord20) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
