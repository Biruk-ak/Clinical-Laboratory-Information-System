...
// LabTestRecord07 represents a domain record in the tests bounded context.
type LabTestRecord07 struct {
...
// Validate performs lightweight domain validation for LabTestRecord07.
func (r *LabTestRecord07) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *LabTestRecord07) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *LabTestRecord07) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
