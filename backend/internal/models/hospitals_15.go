...
// HospitalRecord15 represents a domain record in the hospitals bounded context.
type HospitalRecord15 struct {
...
// Validate performs lightweight domain validation for HospitalRecord15.
func (r *HospitalRecord15) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *HospitalRecord15) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *HospitalRecord15) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
