...
// AnalyticsRecord13 represents a domain record in the analytics bounded context.
type AnalyticsRecord13 struct {
...
// Validate performs lightweight domain validation for AnalyticsRecord13.
func (r *AnalyticsRecord13) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *AnalyticsRecord13) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *AnalyticsRecord13) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
