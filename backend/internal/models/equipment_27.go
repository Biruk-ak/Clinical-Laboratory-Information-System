...
// EquipmentRecord27 represents a domain record in the equipment bounded context.
type EquipmentRecord27 struct {
...
// Validate performs lightweight domain validation for EquipmentRecord27.
func (r *EquipmentRecord27) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *EquipmentRecord27) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *EquipmentRecord27) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
