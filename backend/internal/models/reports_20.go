...
// ReportRecord20 represents a domain record in the reports bounded context.
type ReportRecord20 struct {
...
// Validate performs lightweight domain validation for ReportRecord20.
func (r *ReportRecord20) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *ReportRecord20) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *ReportRecord20) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
