...
// ResultRecord10 represents a domain record in the results bounded context.
type ResultRecord10 struct {
...
// Validate performs lightweight domain validation for ResultRecord10.
func (r *ResultRecord10) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *ResultRecord10) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *ResultRecord10) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
