...
// SampleRecord07 represents a domain record in the samples bounded context.
type SampleRecord07 struct {
...
// Validate performs lightweight domain validation for SampleRecord07.
func (r *SampleRecord07) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *SampleRecord07) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *SampleRecord07) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
