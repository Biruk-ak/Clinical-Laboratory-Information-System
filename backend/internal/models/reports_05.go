...
// ReportRecord05 represents a domain record in the reports bounded context.
type ReportRecord05 struct {
...
// Validate performs lightweight domain validation for ReportRecord05.
func (r *ReportRecord05) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *ReportRecord05) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *ReportRecord05) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
