...
// AuthRecord26 represents a domain record in the auth bounded context.
type AuthRecord26 struct {
...
// Validate performs lightweight domain validation for AuthRecord26.
func (r *AuthRecord26) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *AuthRecord26) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *AuthRecord26) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
