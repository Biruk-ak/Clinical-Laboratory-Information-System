...
// DoctorRecord08 represents a domain record in the doctors bounded context.
type DoctorRecord08 struct {
...
// Validate performs lightweight domain validation for DoctorRecord08.
func (r *DoctorRecord08) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *DoctorRecord08) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *DoctorRecord08) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
