...
// LabTestRecord23 represents a domain record in the tests bounded context.
type LabTestRecord23 struct {
...
// Validate performs lightweight domain validation for LabTestRecord23.
func (r *LabTestRecord23) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *LabTestRecord23) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *LabTestRecord23) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
