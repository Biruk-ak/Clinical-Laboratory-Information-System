...
// PatientRecord05 represents a domain record in the patients bounded context.
type PatientRecord05 struct {
...
// Validate performs lightweight domain validation for PatientRecord05.
func (r *PatientRecord05) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *PatientRecord05) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *PatientRecord05) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
