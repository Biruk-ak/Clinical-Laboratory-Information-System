...
// AnalyticsRecord03 represents a domain record in the analytics bounded context.
type AnalyticsRecord03 struct {
...
// Validate performs lightweight domain validation for AnalyticsRecord03.
func (r *AnalyticsRecord03) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *AnalyticsRecord03) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *AnalyticsRecord03) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
