...
// HospitalRecord12 represents a domain record in the hospitals bounded context.
type HospitalRecord12 struct {
...
// Validate performs lightweight domain validation for HospitalRecord12.
func (r *HospitalRecord12) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *HospitalRecord12) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *HospitalRecord12) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
