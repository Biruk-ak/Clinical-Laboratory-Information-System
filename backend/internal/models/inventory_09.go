...
// InventoryRecord09 represents a domain record in the inventory bounded context.
type InventoryRecord09 struct {
...
// Validate performs lightweight domain validation for InventoryRecord09.
func (r *InventoryRecord09) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *InventoryRecord09) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *InventoryRecord09) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
