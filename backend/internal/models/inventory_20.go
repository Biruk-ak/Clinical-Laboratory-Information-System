...
// InventoryRecord20 represents a domain record in the inventory bounded context.
type InventoryRecord20 struct {
...
// Validate performs lightweight domain validation for InventoryRecord20.
func (r *InventoryRecord20) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *InventoryRecord20) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *InventoryRecord20) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
