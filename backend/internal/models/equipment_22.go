...
// EquipmentRecord22 represents a domain record in the equipment bounded context.
type EquipmentRecord22 struct {
...
// Validate performs lightweight domain validation for EquipmentRecord22.
func (r *EquipmentRecord22) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *EquipmentRecord22) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *EquipmentRecord22) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
