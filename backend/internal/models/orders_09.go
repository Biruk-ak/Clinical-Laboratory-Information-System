...
// OrderRecord09 represents a domain record in the orders bounded context.
type OrderRecord09 struct {
...
// Validate performs lightweight domain validation for OrderRecord09.
func (r *OrderRecord09) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *OrderRecord09) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *OrderRecord09) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
