...
// EquipmentRecord03 represents a domain record in the equipment bounded context.
type EquipmentRecord03 struct {
...
// Validate performs lightweight domain validation for EquipmentRecord03.
func (r *EquipmentRecord03) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *EquipmentRecord03) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *EquipmentRecord03) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
