...
// BillingRecord21 represents a domain record in the billing bounded context.
type BillingRecord21 struct {
...
// Validate performs lightweight domain validation for BillingRecord21.
func (r *BillingRecord21) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *BillingRecord21) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *BillingRecord21) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
