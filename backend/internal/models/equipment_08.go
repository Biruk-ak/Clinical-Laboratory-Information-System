...
// EquipmentRecord08 represents a domain record in the equipment bounded context.
type EquipmentRecord08 struct {
...
// Validate performs lightweight domain validation for EquipmentRecord08.
func (r *EquipmentRecord08) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *EquipmentRecord08) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *EquipmentRecord08) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
