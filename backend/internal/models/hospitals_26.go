...
// HospitalRecord26 represents a domain record in the hospitals bounded context.
type HospitalRecord26 struct {
...
// Validate performs lightweight domain validation for HospitalRecord26.
func (r *HospitalRecord26) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *HospitalRecord26) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *HospitalRecord26) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
