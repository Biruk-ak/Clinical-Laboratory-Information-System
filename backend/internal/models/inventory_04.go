...
// InventoryRecord04 represents a domain record in the inventory bounded context.
type InventoryRecord04 struct {
...
// Validate performs lightweight domain validation for InventoryRecord04.
func (r *InventoryRecord04) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *InventoryRecord04) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *InventoryRecord04) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
