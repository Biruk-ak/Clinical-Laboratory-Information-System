...
// OrderRecord22 represents a domain record in the orders bounded context.
type OrderRecord22 struct {
...
// Validate performs lightweight domain validation for OrderRecord22.
func (r *OrderRecord22) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *OrderRecord22) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *OrderRecord22) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
