...
// AnalyticsRecord22 represents a domain record in the analytics bounded context.
type AnalyticsRecord22 struct {
...
// Validate performs lightweight domain validation for AnalyticsRecord22.
func (r *AnalyticsRecord22) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *AnalyticsRecord22) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *AnalyticsRecord22) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
