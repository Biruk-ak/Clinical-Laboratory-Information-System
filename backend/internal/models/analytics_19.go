...
// AnalyticsRecord19 represents a domain record in the analytics bounded context.
type AnalyticsRecord19 struct {
...
// Validate performs lightweight domain validation for AnalyticsRecord19.
func (r *AnalyticsRecord19) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *AnalyticsRecord19) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *AnalyticsRecord19) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
