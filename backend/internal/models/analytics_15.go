...
// AnalyticsRecord15 represents a domain record in the analytics bounded context.
type AnalyticsRecord15 struct {
...
// Validate performs lightweight domain validation for AnalyticsRecord15.
func (r *AnalyticsRecord15) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *AnalyticsRecord15) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *AnalyticsRecord15) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
