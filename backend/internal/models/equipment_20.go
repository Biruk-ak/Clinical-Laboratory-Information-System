...
// EquipmentRecord20 represents a domain record in the equipment bounded context.
type EquipmentRecord20 struct {
...
// Validate performs lightweight domain validation for EquipmentRecord20.
func (r *EquipmentRecord20) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *EquipmentRecord20) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *EquipmentRecord20) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
