...
// AnalyticsRecord01 represents a domain record in the analytics bounded context.
type AnalyticsRecord01 struct {
...
// Validate performs lightweight domain validation for AnalyticsRecord01.
func (r *AnalyticsRecord01) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *AnalyticsRecord01) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *AnalyticsRecord01) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
