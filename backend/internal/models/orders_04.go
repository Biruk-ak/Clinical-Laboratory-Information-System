...
// OrderRecord04 represents a domain record in the orders bounded context.
type OrderRecord04 struct {
...
// Validate performs lightweight domain validation for OrderRecord04.
func (r *OrderRecord04) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *OrderRecord04) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *OrderRecord04) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
