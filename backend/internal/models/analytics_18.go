...
// AnalyticsRecord18 represents a domain record in the analytics bounded context.
type AnalyticsRecord18 struct {
...
// Validate performs lightweight domain validation for AnalyticsRecord18.
func (r *AnalyticsRecord18) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *AnalyticsRecord18) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *AnalyticsRecord18) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
