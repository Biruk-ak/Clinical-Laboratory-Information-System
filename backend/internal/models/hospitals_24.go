...
// HospitalRecord24 represents a domain record in the hospitals bounded context.
type HospitalRecord24 struct {
...
// Validate performs lightweight domain validation for HospitalRecord24.
func (r *HospitalRecord24) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *HospitalRecord24) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *HospitalRecord24) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
