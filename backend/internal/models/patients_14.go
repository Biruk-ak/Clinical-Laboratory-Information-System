...
// PatientRecord14 represents a domain record in the patients bounded context.
type PatientRecord14 struct {
...
// Validate performs lightweight domain validation for PatientRecord14.
func (r *PatientRecord14) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *PatientRecord14) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *PatientRecord14) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
