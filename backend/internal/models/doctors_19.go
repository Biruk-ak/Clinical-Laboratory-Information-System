...
// DoctorRecord19 represents a domain record in the doctors bounded context.
type DoctorRecord19 struct {
...
// Validate performs lightweight domain validation for DoctorRecord19.
func (r *DoctorRecord19) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *DoctorRecord19) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *DoctorRecord19) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
