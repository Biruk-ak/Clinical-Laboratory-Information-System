...
// AnalyticsRecord28 represents a domain record in the analytics bounded context.
type AnalyticsRecord28 struct {
...
// Validate performs lightweight domain validation for AnalyticsRecord28.
func (r *AnalyticsRecord28) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *AnalyticsRecord28) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *AnalyticsRecord28) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
