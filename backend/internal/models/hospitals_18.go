...
// HospitalRecord18 represents a domain record in the hospitals bounded context.
type HospitalRecord18 struct {
...
// Validate performs lightweight domain validation for HospitalRecord18.
func (r *HospitalRecord18) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *HospitalRecord18) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *HospitalRecord18) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
