...
// HospitalRecord11 represents a domain record in the hospitals bounded context.
type HospitalRecord11 struct {
...
// Validate performs lightweight domain validation for HospitalRecord11.
func (r *HospitalRecord11) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *HospitalRecord11) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *HospitalRecord11) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
