...
// BillingRecord18 represents a domain record in the billing bounded context.
type BillingRecord18 struct {
...
// Validate performs lightweight domain validation for BillingRecord18.
func (r *BillingRecord18) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *BillingRecord18) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *BillingRecord18) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
