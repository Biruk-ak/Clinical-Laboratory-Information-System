...
// InventoryRecord07 represents a domain record in the inventory bounded context.
type InventoryRecord07 struct {
...
// Validate performs lightweight domain validation for InventoryRecord07.
func (r *InventoryRecord07) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *InventoryRecord07) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *InventoryRecord07) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
