...
// DoctorRecord09 represents a domain record in the doctors bounded context.
type DoctorRecord09 struct {
...
// Validate performs lightweight domain validation for DoctorRecord09.
func (r *DoctorRecord09) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *DoctorRecord09) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *DoctorRecord09) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
