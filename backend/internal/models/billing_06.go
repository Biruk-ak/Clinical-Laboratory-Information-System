...
// BillingRecord06 represents a domain record in the billing bounded context.
type BillingRecord06 struct {
...
// Validate performs lightweight domain validation for BillingRecord06.
func (r *BillingRecord06) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *BillingRecord06) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *BillingRecord06) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
