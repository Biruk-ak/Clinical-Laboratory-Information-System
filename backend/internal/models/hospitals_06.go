...
// HospitalRecord06 represents a domain record in the hospitals bounded context.
type HospitalRecord06 struct {
...
// Validate performs lightweight domain validation for HospitalRecord06.
func (r *HospitalRecord06) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *HospitalRecord06) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *HospitalRecord06) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
