...
// DoctorRecord07 represents a domain record in the doctors bounded context.
type DoctorRecord07 struct {
...
// Validate performs lightweight domain validation for DoctorRecord07.
func (r *DoctorRecord07) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *DoctorRecord07) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *DoctorRecord07) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
