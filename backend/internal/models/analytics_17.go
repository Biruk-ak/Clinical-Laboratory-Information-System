...
// AnalyticsRecord17 represents a domain record in the analytics bounded context.
type AnalyticsRecord17 struct {
...
// Validate performs lightweight domain validation for AnalyticsRecord17.
func (r *AnalyticsRecord17) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *AnalyticsRecord17) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *AnalyticsRecord17) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
