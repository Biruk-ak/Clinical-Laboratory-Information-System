...
// EquipmentRecord12 represents a domain record in the equipment bounded context.
type EquipmentRecord12 struct {
...
// Validate performs lightweight domain validation for EquipmentRecord12.
func (r *EquipmentRecord12) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *EquipmentRecord12) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *EquipmentRecord12) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
