...
// DoctorRecord03 represents a domain record in the doctors bounded context.
type DoctorRecord03 struct {
...
// Validate performs lightweight domain validation for DoctorRecord03.
func (r *DoctorRecord03) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *DoctorRecord03) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *DoctorRecord03) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
