...
// DoctorRecord20 represents a domain record in the doctors bounded context.
type DoctorRecord20 struct {
...
// Validate performs lightweight domain validation for DoctorRecord20.
func (r *DoctorRecord20) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *DoctorRecord20) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *DoctorRecord20) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
