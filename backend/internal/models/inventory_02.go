...
// InventoryRecord02 represents a domain record in the inventory bounded context.
type InventoryRecord02 struct {
...
// Validate performs lightweight domain validation for InventoryRecord02.
func (r *InventoryRecord02) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *InventoryRecord02) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *InventoryRecord02) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
