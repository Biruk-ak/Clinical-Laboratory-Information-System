...
// AnalyticsRecord26 represents a domain record in the analytics bounded context.
type AnalyticsRecord26 struct {
...
// Validate performs lightweight domain validation for AnalyticsRecord26.
func (r *AnalyticsRecord26) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *AnalyticsRecord26) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *AnalyticsRecord26) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
