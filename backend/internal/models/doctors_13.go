...
// DoctorRecord13 represents a domain record in the doctors bounded context.
type DoctorRecord13 struct {
...
// Validate performs lightweight domain validation for DoctorRecord13.
func (r *DoctorRecord13) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *DoctorRecord13) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *DoctorRecord13) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
