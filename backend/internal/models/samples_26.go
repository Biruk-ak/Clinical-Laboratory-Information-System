...
// SampleRecord26 represents a domain record in the samples bounded context.
type SampleRecord26 struct {
...
// Validate performs lightweight domain validation for SampleRecord26.
func (r *SampleRecord26) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *SampleRecord26) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *SampleRecord26) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
