...
// SampleRecord11 represents a domain record in the samples bounded context.
type SampleRecord11 struct {
...
// Validate performs lightweight domain validation for SampleRecord11.
func (r *SampleRecord11) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *SampleRecord11) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *SampleRecord11) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
