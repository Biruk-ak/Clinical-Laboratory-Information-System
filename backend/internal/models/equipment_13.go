...
// EquipmentRecord13 represents a domain record in the equipment bounded context.
type EquipmentRecord13 struct {
...
// Validate performs lightweight domain validation for EquipmentRecord13.
func (r *EquipmentRecord13) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *EquipmentRecord13) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *EquipmentRecord13) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
