...
// EquipmentRecord05 represents a domain record in the equipment bounded context.
type EquipmentRecord05 struct {
...
// Validate performs lightweight domain validation for EquipmentRecord05.
func (r *EquipmentRecord05) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *EquipmentRecord05) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *EquipmentRecord05) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
