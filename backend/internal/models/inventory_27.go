...
// InventoryRecord27 represents a domain record in the inventory bounded context.
type InventoryRecord27 struct {
...
// Validate performs lightweight domain validation for InventoryRecord27.
func (r *InventoryRecord27) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *InventoryRecord27) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *InventoryRecord27) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
