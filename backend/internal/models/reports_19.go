...
// ReportRecord19 represents a domain record in the reports bounded context.
type ReportRecord19 struct {
...
// Validate performs lightweight domain validation for ReportRecord19.
func (r *ReportRecord19) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *ReportRecord19) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *ReportRecord19) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
