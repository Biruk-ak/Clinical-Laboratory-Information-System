...
// OrderRecord19 represents a domain record in the orders bounded context.
type OrderRecord19 struct {
...
// Validate performs lightweight domain validation for OrderRecord19.
func (r *OrderRecord19) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *OrderRecord19) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *OrderRecord19) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
