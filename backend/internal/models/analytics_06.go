...
// AnalyticsRecord06 represents a domain record in the analytics bounded context.
type AnalyticsRecord06 struct {
...
// Validate performs lightweight domain validation for AnalyticsRecord06.
func (r *AnalyticsRecord06) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *AnalyticsRecord06) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *AnalyticsRecord06) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
