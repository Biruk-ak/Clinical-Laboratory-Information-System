...
// DoctorRecord15 represents a domain record in the doctors bounded context.
type DoctorRecord15 struct {
...
// Validate performs lightweight domain validation for DoctorRecord15.
func (r *DoctorRecord15) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *DoctorRecord15) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *DoctorRecord15) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
