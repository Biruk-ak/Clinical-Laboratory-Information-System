...
// SampleRecord24 represents a domain record in the samples bounded context.
type SampleRecord24 struct {
...
// Validate performs lightweight domain validation for SampleRecord24.
func (r *SampleRecord24) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *SampleRecord24) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *SampleRecord24) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
