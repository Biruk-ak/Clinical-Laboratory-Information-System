...
// PatientRecord08 represents a domain record in the patients bounded context.
type PatientRecord08 struct {
...
// Validate performs lightweight domain validation for PatientRecord08.
func (r *PatientRecord08) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *PatientRecord08) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *PatientRecord08) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
