...
// BillingRecord24 represents a domain record in the billing bounded context.
type BillingRecord24 struct {
...
// Validate performs lightweight domain validation for BillingRecord24.
func (r *BillingRecord24) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *BillingRecord24) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *BillingRecord24) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
