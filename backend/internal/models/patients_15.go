...
// PatientRecord15 represents a domain record in the patients bounded context.
type PatientRecord15 struct {
...
// Validate performs lightweight domain validation for PatientRecord15.
func (r *PatientRecord15) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *PatientRecord15) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *PatientRecord15) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
