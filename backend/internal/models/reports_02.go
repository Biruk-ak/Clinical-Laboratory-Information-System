...
// ReportRecord02 represents a domain record in the reports bounded context.
type ReportRecord02 struct {
...
// Validate performs lightweight domain validation for ReportRecord02.
func (r *ReportRecord02) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *ReportRecord02) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *ReportRecord02) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
