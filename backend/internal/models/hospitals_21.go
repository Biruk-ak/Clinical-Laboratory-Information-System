...
// HospitalRecord21 represents a domain record in the hospitals bounded context.
type HospitalRecord21 struct {
...
// Validate performs lightweight domain validation for HospitalRecord21.
func (r *HospitalRecord21) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *HospitalRecord21) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *HospitalRecord21) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
