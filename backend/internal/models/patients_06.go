...
// PatientRecord06 represents a domain record in the patients bounded context.
type PatientRecord06 struct {
...
// Validate performs lightweight domain validation for PatientRecord06.
func (r *PatientRecord06) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *PatientRecord06) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *PatientRecord06) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
