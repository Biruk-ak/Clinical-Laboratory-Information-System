...
// DoctorRecord10 represents a domain record in the doctors bounded context.
type DoctorRecord10 struct {
...
// Validate performs lightweight domain validation for DoctorRecord10.
func (r *DoctorRecord10) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *DoctorRecord10) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *DoctorRecord10) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
