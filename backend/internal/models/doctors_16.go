...
// DoctorRecord16 represents a domain record in the doctors bounded context.
type DoctorRecord16 struct {
...
// Validate performs lightweight domain validation for DoctorRecord16.
func (r *DoctorRecord16) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *DoctorRecord16) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *DoctorRecord16) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
