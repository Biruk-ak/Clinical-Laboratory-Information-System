...
// OrderRecord05 represents a domain record in the orders bounded context.
type OrderRecord05 struct {
...
// Validate performs lightweight domain validation for OrderRecord05.
func (r *OrderRecord05) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *OrderRecord05) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *OrderRecord05) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
