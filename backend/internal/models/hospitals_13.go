...
// HospitalRecord13 represents a domain record in the hospitals bounded context.
type HospitalRecord13 struct {
...
// Validate performs lightweight domain validation for HospitalRecord13.
func (r *HospitalRecord13) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *HospitalRecord13) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *HospitalRecord13) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
