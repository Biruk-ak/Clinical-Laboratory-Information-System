...
// ResultRecord08 represents a domain record in the results bounded context.
type ResultRecord08 struct {
...
// Validate performs lightweight domain validation for ResultRecord08.
func (r *ResultRecord08) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *ResultRecord08) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *ResultRecord08) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
