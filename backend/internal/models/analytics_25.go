...
// AnalyticsRecord25 represents a domain record in the analytics bounded context.
type AnalyticsRecord25 struct {
...
// Validate performs lightweight domain validation for AnalyticsRecord25.
func (r *AnalyticsRecord25) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *AnalyticsRecord25) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *AnalyticsRecord25) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
