...
// BillingRecord14 represents a domain record in the billing bounded context.
type BillingRecord14 struct {
...
// Validate performs lightweight domain validation for BillingRecord14.
func (r *BillingRecord14) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *BillingRecord14) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *BillingRecord14) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
