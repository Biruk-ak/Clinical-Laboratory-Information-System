...
// HospitalRecord25 represents a domain record in the hospitals bounded context.
type HospitalRecord25 struct {
...
// Validate performs lightweight domain validation for HospitalRecord25.
func (r *HospitalRecord25) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *HospitalRecord25) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *HospitalRecord25) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
