...
// InventoryRecord10 represents a domain record in the inventory bounded context.
type InventoryRecord10 struct {
...
// Validate performs lightweight domain validation for InventoryRecord10.
func (r *InventoryRecord10) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *InventoryRecord10) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *InventoryRecord10) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
