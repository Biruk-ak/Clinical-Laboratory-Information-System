...
// ResultRecord07 represents a domain record in the results bounded context.
type ResultRecord07 struct {
...
// Validate performs lightweight domain validation for ResultRecord07.
func (r *ResultRecord07) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *ResultRecord07) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *ResultRecord07) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
