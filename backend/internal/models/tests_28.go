...
// LabTestRecord28 represents a domain record in the tests bounded context.
type LabTestRecord28 struct {
...
// Validate performs lightweight domain validation for LabTestRecord28.
func (r *LabTestRecord28) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *LabTestRecord28) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *LabTestRecord28) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
