...
// AnalyticsRecord05 represents a domain record in the analytics bounded context.
type AnalyticsRecord05 struct {
...
// Validate performs lightweight domain validation for AnalyticsRecord05.
func (r *AnalyticsRecord05) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *AnalyticsRecord05) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *AnalyticsRecord05) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
