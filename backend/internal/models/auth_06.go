...
// AuthRecord06 represents a domain record in the auth bounded context.
type AuthRecord06 struct {
...
// Validate performs lightweight domain validation for AuthRecord06.
func (r *AuthRecord06) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *AuthRecord06) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *AuthRecord06) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
