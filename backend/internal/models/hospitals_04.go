...
// HospitalRecord04 represents a domain record in the hospitals bounded context.
type HospitalRecord04 struct {
...
// Validate performs lightweight domain validation for HospitalRecord04.
func (r *HospitalRecord04) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *HospitalRecord04) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *HospitalRecord04) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
