...
// DoctorRecord01 represents a domain record in the doctors bounded context.
type DoctorRecord01 struct {
...
// Validate performs lightweight domain validation for DoctorRecord01.
func (r *DoctorRecord01) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *DoctorRecord01) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *DoctorRecord01) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
