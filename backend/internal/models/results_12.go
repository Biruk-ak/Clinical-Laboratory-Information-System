...
// ResultRecord12 represents a domain record in the results bounded context.
type ResultRecord12 struct {
...
// Validate performs lightweight domain validation for ResultRecord12.
func (r *ResultRecord12) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *ResultRecord12) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *ResultRecord12) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
