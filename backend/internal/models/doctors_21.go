...
// DoctorRecord21 represents a domain record in the doctors bounded context.
type DoctorRecord21 struct {
...
// Validate performs lightweight domain validation for DoctorRecord21.
func (r *DoctorRecord21) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *DoctorRecord21) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *DoctorRecord21) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
