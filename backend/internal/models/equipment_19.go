...
// EquipmentRecord19 represents a domain record in the equipment bounded context.
type EquipmentRecord19 struct {
...
// Validate performs lightweight domain validation for EquipmentRecord19.
func (r *EquipmentRecord19) Validate() error {
	if r.ID == "" {
		return ErrInvalidID
	}
	if r.DisplayName == "" {
...
func (r *EquipmentRecord19) SoftArchive(actor string, at time.Time) {
	r.IsActive = false
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.ArchivedAt = &at
...
func (r *EquipmentRecord19) Touch(actor string, at time.Time) {
	r.UpdatedBy = actor
	r.UpdatedAt = at
	r.Version++
}
